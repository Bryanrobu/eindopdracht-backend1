<!DOCTYPE html>
<html lang="nl">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gerecht Bewerken</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="admin-body">

    @include('partials.header')

    <main class="admin-main">
        <div class="admin-card">

            <div class="admin-header">
                <h1 class="admin-title">Gerecht Bewerken</h1>
                <a href="{{ route('items.index') }}" class="text-light">Terug naar overzicht</a>
            </div>

            <form action="{{ route('items.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="admin-form-group">
                    <label class="admin-label">Naam</label>
                    <input type="text" name="name" value="{{ $item->name }}" class="admin-input" required>
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Categorie</label>
                    <input type="text" name="category" value="{{ $item->category }}" class="admin-input" required>
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Omschrijving</label>
                    <textarea name="description" rows="4" class="admin-textarea">{{ $item->description }}</textarea>
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Prijs (€)</label>
                    <input type="number" step="0.01" name="price" value="{{ $item->price }}" class="admin-input"
                        required>
                </div>

                <button type="submit" class="btn-secondary btn-block">Wijzigingen Opslaan</button>
            </form>
        </div>
    </main>
    @include('partials.footer')
</body>

</html>